...
class BaseHandler(RequestHandler):
    ...
    def write(self, chunk: Union[str, bytes, dict], ensure_json: bool = True) -> None:
        """write 字典时自动转换成 json"""
...
class CheckLoggedMixin:
    ...
    def check_login(self: Union[RequestHandler, WebSocketHandler]) -> Tuple[str, bool]:
        """用于检查是否已经登入"""
        user_name = self.get_secure_cookie("uname")
        user_ip = "管理员" if self.request.remote_ip == LOCAL_IP else self.request.remote_ip
...
class UserLogin(CheckLoggedMixin, MustLoginMixin, BaseHandler):
    async def get(self):
        ...
        params = {"title": "登入", "notice_message": "", "logged_in": logged_in, "logging_in": True}
    ...
    def post(self):
        ...
        cur = userdb_cursor.execute("SELECT password FROM user WHERE name=(?)", (uname,))
        r = cur.fetchone()
        if r:
            password = r[0]
            if password == md5pass:
                ...
                params = dict(
                    title="登入", logging_in=True, logged_in=False, notice_message="输入正确的用户名或密码"
                )
                return self.render("login.html", **params)
        else:
...
class UserRegis(CheckLoggedMixin, MustLoginMixin, BaseHandler):
    async def get(self):
        _, logged_in = self.check_login()
        params = {"notice_message": "", "logging_in": True, "logged_in": logged_in}
        return self.render("regis.html", **params)

    def post(self):
        _, logged_in = self.check_login()
        params = {"notice_message": "此用户名已存在", "logging_in": True, "logged_in": logged_in}
        uname = self.get_argument("uname", None)
        with userdb_conn:
            cur = userdb_conn.execute("SELECT name FROM user WHERE name == (?)", (uname,))
        if cur.fetchone():
            return self.render("regis.html", **params)
...
class ChatHandler(CheckLoggedMixin, WebSocketHandler):
    ...
    sql_conn = sqlite3.connect(
        f"file:logs/{time.strftime('%Y-%m-%d', time.localtime())}.db3?mode=rwc", uri=True
    )
    ...
    def open(self):
        ...
        message_str = "[%s]-[%s]-进入聊天室，当前在线人数 [%s]" % (user_ip, current_time, len(self.users))
        for user in self.users:  # 向已在线用户发送消息
            user.write_message(message_str)
        for history_msg in self.sql_cursor.execute(
            "SELECT msg FROM messages WHERE msg_id > " "(SELECT count(msg_id) FROM messages) - ?;",
            (tornado_settings.getint("message_buffer_size"),),
        ):  # noqa
    ...
    def on_message(self, message):
        user_ip, _ = self.check_login()
        current_time = datetime.datetime.now().strftime(self.DATETIME_FORMAT)
        message_str = "[%s]-[%s]-说：%s" % (user_ip, current_time, message)
    ...
    def on_close(self):
        ...
        user_ip, _ = self.check_login()
        current_time = datetime.datetime.now().strftime(self.DATETIME_FORMAT)
        message_str = "[%s]-[%s]-离开聊天室，当前在线人数 [%s]" % (user_ip, current_time, len(self.users))
    ...
    def update_log(cls, chat):
        try:
            with cls.sql_conn:
                cls.sql_conn.execute("INSERT INTO messages VALUES (?, ?)", (chat, None))
    ...
    def check_origin(self, origin):
        ...
        if net_loc.endswith(self.LOCAL_ADDR) or net_loc.endswith(self.HOST_ADDR):
            return True
        return False
...
class ChatPageHandler(CheckLoggedMixin, BaseHandler):
    ...
    def get(self, *args, **kwargs):
        ...
        params = {"user_name": user_name, "logged_in": logged_in, "logging_in": False}
...
class CVHandler(CheckLoggedMixin, WebSocketHandler):
    ...
    def return_img(self, which_img: str = ""):
        ...
        if img is not None:
            ...
            resp = {"mode": "run", "msg": f"There is not an argument named {which_img}"}
    ...
    def on_message(self, message):
        ...
        if message["mode"] == "run":
            ...
            builtin_func = re.search(r"(?P<func>imread|help|show)\((?P<arg>.+)\)", recv_code)
            if builtin_func is not None:
                ...
                if func_type == "imread":
                    ...
                    recv_code = f"pydoc.render_doc({func_arg}, " "renderer=pydoc.plaintext)"
...
class UploadFileHandler(CheckLoggedMixin, MustLoginMixin, BaseHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 文件的暂存路径
        self.upload_path = opth.join(opth.dirname(__file__), tornado_settings["file_upload_dir"])
        if not opth.exists(self.upload_path) or not opth.isdir(self.upload_path):
    ...
    def get(self, *args, **kwargs):
        ...
        files = list(filter(lambda item: opth.isfile(opth.join(self.upload_path, item)), files))
        sizes = list(map(lambda item: opth.getsize(opth.join(self.upload_path, item)), files))
        sizes = list(map(self.get_size, sizes))
        param_keys = ("dir_name", "files", "sizes", "logged_in", "logging_in")
        param_values = (opth.split(self.upload_path)[-1], files, sizes, logged_in, False)
